...
func RedisScan(info *common.HostsInfo) (tmperr error) {
...
func RedisConn(info *common.HostsInfo, pass string) (flag bool, err error) {
	flag = false
...
func RedisUnauth(info *common.HostsInfo) (flag bool, err error) {
	flag = false
	realhost := fmt.Sprintf("%s:%v", info.Host, info.Ports)
	conn, err := common.WrapperTcpWithTimeout("tcp", realhost, time.Duration(common.Timeout)*time.Second)
	if err != nil {
